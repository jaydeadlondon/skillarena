(function () {
  ...
  function render() {
    const percent = Math.min(
      100,
      Math.floor((localWatched * 100) / Math.max(cfg.durationSeconds, 1)),
    );
    if (watchedInput) {
      watchedInput.value = String(localWatched);
    }
    if (watchedMinutes) {
      watchedMinutes.textContent = (localWatched / 60).toFixed(1);
    }
    if (watchedPercent) {
      watchedPercent.textContent = String(percent);
    }
    if (watchedProgress) {
      watchedProgress.style.width = `${percent}%`;
    }
  }
  ...
  if (cfg.provider === "youtube") {
    ...
    btn.addEventListener("click", () => {
      ...
      btn.textContent = fallbackTracking
        ? "Pause fallback lesson timer"
        : "Start fallback lesson timer";
      setStatus(
        fallbackTracking
          ? "Fallback tracking active"
          : "Fallback tracking paused",
      );
      if (!fallbackTracking && pendingSeconds > 0) {
        sendLessonHeartbeat(pendingSeconds);
        pendingSeconds = 0;
      }
    });
