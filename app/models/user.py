...
class User(Base):
    ...
    steam_playtime_2w_minutes: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    created_at: Mapped[created_at]
    updated_at: Mapped[updated_at]

    progresses = relationship("LessonProgress", back_populates="user", cascade="all, delete-orphan")
    quest_progresses = relationship("UserQuest", back_populates="user", cascade="all, delete-orphan")
    ...
    streak_days = relationship("UserStreakDay", back_populates="user", cascade="all, delete-orphan")
    activity_days = relationship("UserActivityDay", back_populates="user", cascade="all, delete-orphan")
